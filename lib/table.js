...
Table.prototype.delete = function(key, value, cascade) {
	...
	for (var idx in indexs) {
		_this.database.tables.forEach(function(table) {
			var temp = _this.database.get(table);
			temp.columns.forEach(function(col) {
				if (_this.name == col) {
					if (cascade != undefined && cascade == true) {
						_this.delete(_this.name, idx);
					} else {
						throw 'Cannot delete a data with reference in ' + temp.name;
					}
				}
			});
		});
		_this.lines.splice(idx, 1);
	}
 ...
 * @param idx Numéro de la ligne
 * @param cascade Suppression en cascade (facultatif) */
Table.prototype.deleteID = function(idx, cascade) {
    this.database.log('...DELETE FROM [' + this.name + '] WHERE ID = [' + idx + ']');
	this.read();
	var _this = this;
	this.database.tables.forEach(function(table) {
		var temp = _this.database.get(table);
		temp.columns.forEach(function(col) {
			if (_this.name == col) {
				if (cascade != undefined && cascade == true) {
